...
JAVA=java
INPUTDIR=/wikidata/WikidumpXMLEnglish/plaintext
LUCENEINDEXDIR=/wikidata/WikidumpXMLEnglish/indexes/engpos

i=0; 
j=1000;
#for j in `ls $INPUTDIR/$i`; do
        echo "Processig: $i - $j "
        sed "s#__INPUTDIRECTORY#$INPUTDIR/$i/$j#; s#__LUCENEINDEXDIR#$LUCENEINDEXDIR#" $WD/descriptors/CPE/engPosIndexer_CPE.xml  > $WD/descriptors/CPE/cpe_current.xml
