...
LUCENEINDEXDIR=/wikidata/WikidumpHungarian/indexes/hunstem
XCASOUTPUTDIR=/wikidata/WikidumpHungarian/output/xcasout
...
for i in `ls $INPUTDIR`; do
   for j in `ls $INPUTDIR/$i`; do
        echo "Processig: $i - $j "
        mkdir -p $XCASOUTPUTDIR/$i/$j;
        sed "s#__INPUTDIR#$INPUTDIR/$i/$j#; s#__LUCENEINDEXDIR#$LUCENEINDEXDIR#; s#__XCASOUTPUTDIR#$XCASOUTPUTDIR/$i/$j#" $WD/descriptors/CPE/hunParserStemmerIndexer_CPE.xml  > $WD/descriptors/CPE/cpe_current.xml
